...
async fn start_global_event_generation(state: Arc<AppState>) {
    ...
    loop {
        ...
        // Generate random events - force more AuctionCompleted for blockchain demo
        let event_type = match rand::random::<u8>() % 5 {
            0 => "AuctionStarted",
            1 => "AuctionCompleted", // 20% chance
            2 => "AuctionCompleted", // 20% chance (doubled)
            3 => "SystemMetrics",
            4 => "SystemMetrics",
            _ => "SystemMetrics",
        };
        ...
        let event = match event_type {
            ...
            "AuctionCompleted" => {
                ...
                // Log blockchain settlement
                info!("🔗 Blockchain settlement triggered for auction #{}", auction_counter);
                
                // Create real blockchain transaction
                let blockchain_client = state.blockchain_client.read().await;
                if let Some(ref client) = *blockchain_client {
                    // Generate keypairs for this settlement (in production, these would be real user keypairs)
                    let aggregator_keypair = solana_sdk::signature::Keypair::new();
                    let battery_keypair = solana_sdk::signature::Keypair::new();
                    
                    // Convert energy from kWh to Wh (Solana uses Wh for precision)
                    let energy_wh = (energy * 1000.0) as u64;
                    let price_cents = price as u64;
                    
                    // First initialize aggregator and battery accounts
                    match client.initialize_aggregator(&aggregator_keypair).await {
                        Ok(_) => {
                            info!("✅ Aggregator initialized on blockchain");
                        }
                        Err(e) => {
                            warn!("⚠️ Failed to initialize aggregator: {}", e);
                        }
                    }
                    
                    match client.initialize_battery(&battery_keypair).await {
                        Ok(_) => {
                            info!("✅ Battery initialized on blockchain");
                        }
                        Err(e) => {
                            warn!("⚠️ Failed to initialize battery: {}", e);
                        }
                    }
                    ...
                    match client.initialize_auction(
                        ...
                        Ok(auction_pubkey) => {
                            info!("✅ Auction created on blockchain: {}", auction_pubkey);
                            ...
                            match client.settle_auction(
                                ...
                                Ok(signature) => {
                                    info!("✅ Real blockchain transaction sent: {}", signature);
                                    
                                    // Generate blockchain settlement event with REAL signature
                                    let settlement_event = serde_json::json!({
                                        ...
                                        "data": {
                                            ...
                                            "settlement_signature": signature.to_string(),
                                            "blockchain_url": client.get_local_transaction_url(&signature),
                                            "timestamp": current_timestamp()
                                        },
                                        "timestamp": current_timestamp()
                                    });
                                    
                                    // Broadcast the blockchain settlement event
                                    if let Err(e) = state.event_tx.send(serde_json::from_value(settlement_event).unwrap()) {
                                        warn!("Failed to broadcast blockchain settlement event: {}", e);
                                    }
                                }
                                Err(e) => {
                                    error!("❌ Failed to send blockchain transaction: {}", e);
                                    
                                    // Generate error settlement event
                                    let settlement_event = serde_json::json!({
                                        "type": "BlockchainSettlement",
                                        "data": {
                                            "auction_id": auction_counter,
                                            ...
                                            "final_price": price,
                                            "total_value": energy * price as f64,
                                            "settlement_signature": "BLOCKCHAIN_ERROR",
                                            "error": e.to_string(),
                                            "timestamp": current_timestamp()
                                        },
                                        "timestamp": current_timestamp()
                                    ...
                                    // Broadcast the error settlement event
                                    if let Err(e) = state.event_tx.send(serde_json::from_value(settlement_event).unwrap()) {
                                        warn!("Failed to broadcast blockchain settlement event: {}", e);
                                    }
                                }
                        ...
                        Err(e) => {
                            error!("❌ Failed to create auction on blockchain: {}", e);
                        }
                    }
                } else {
                    warn!("⚠️ Blockchain client not available - skipping real transaction");
                    
                    // Generate mock settlement event when blockchain is not available
                    let settlement_event = serde_json::json!({
                        "type": "BlockchainSettlement",
                        "data": {
                            "auction_id": auction_counter,
                            ...
                            "final_price": price,
                            "total_value": energy * price as f64,
                            "settlement_signature": "BLOCKCHAIN_UNAVAILABLE",
                            "timestamp": current_timestamp()
                        },
                        "timestamp": current_timestamp()
                    });
                    
                    // Broadcast the mock settlement event
                    if let Err(e) = state.event_tx.send(serde_json::from_value(settlement_event).unwrap()) {
                        warn!("Failed to broadcast blockchain settlement event: {}", e);
                    }
                }
